...
class IMBankCompleteCrawler:
    ...
    def go_to_main_page_and_find_iframe(self):
        ...
        self.logger.info("🔄 메인 페이지로 이동하고 iframe 찾는 중...")
        
        main_url = "https://www.imbank.co.kr/com_ebz_fpm_main.act"
        self.logger.info(f"📍 메인 페이지 이동: {main_url}")
        ...
        # iframe 찾기
        self.logger.info("🔍 iframe (#ifr) 찾는 중...")
        
        try:
            iframe = WebDriverWait(self.driver, 20).until(
                EC.presence_of_element_located((By.ID, "ifr"))
            )
            self.logger.info("✅ iframe 발견!")
            
            iframe_src = iframe.get_attribute('src')
            self.logger.info(f"📍 초기 iframe src: '{iframe_src}'")
            
            # iframe이 비어있는 것이 정상 상태
            if not iframe_src or iframe_src.strip() == '':
                self.logger.info("✅ iframe이 비어있음 - 이것이 정상 상태입니다")
                return True
                
        except Exception as e:
            self.logger.info(f"❌ iframe 찾기 실패: {e}")
            return False
    
    def navigate_to_category_in_iframe(self, category, url_code):
        """iframe에 카테고리 URL 설정"""
        self.logger.info(f"🔄 iframe에 {category} URL 설정 중...")
        
        category_url = f"https://www.imbank.co.kr/fnp_ebz_{url_code}_depo.act"
        self.logger.info(f"📍 설정할 URL: {category_url}")
        ...
        try:
            ...
            if not iframe_exists:
                self.logger.info("⚠️ iframe이 존재하지 않음 - 페이지 새로고침 시도")
                main_url = "https://www.imbank.co.kr/com_ebz_fpm_main.act"
                self.driver.get(main_url)
                WebDriverWait(self.driver, 30).until(
                    lambda driver: driver.execute_script("return document.readyState") == "complete"
            ...
            self.logger.info(f"✅ iframe URL 설정 완료")
            ...
            self.logger.info("✅ iframe 내부로 전환 완료")
            
            # 카테고리 페이지 로딩 확인
            return self.wait_for_iframe_complete()
            
        except Exception as e:
            self.logger.info(f"❌ iframe URL 설정 실패: {e}")
            return False
    
    def wait_for_iframe_complete(self):
        """iframe 완전 로딩 대기"""
        self.logger.info("🔄 iframe 완전 로딩 대기 중...")
        ...
        for attempt in range(1, 21):
            try:
                ...
                self.logger.info(f"⏳ 시도 {attempt}/20: 텍스트 {len(body_text)}자, HTML {len(body_html)}자, 상품키워드: {has_content}")
                
                if text_sufficient or html_sufficient or (len(body_text) > 200 and has_content):
                    self.logger.info(f"✅ iframe 완전 로딩 완료 ({attempt}회 시도)")
                    time.sleep(2)
                    return True
                    
            except Exception as e:
                self.logger.info(f"⏳ iframe 로딩 대기 중... ({attempt}/20) - 오류: {e}")
            
            time.sleep(3)
        
        self.logger.info("❌ iframe 로딩 시간 초과")
        return False
    
    def show_all_products_in_one_page(self):
        """한 페이지에 모든 상품 표시"""
        try:
            self.logger.info("📋 한 페이지에 모든 상품 표시 시도...")
            
            body_text = self.driver.find_element(By.TAG_NAME, "body").text
            total_match = re.search(r'총\s*(\d+)\s*건', body_text)
            expected_total = int(total_match.group(1)) if total_match else 50
            
            self.logger.info(f"📊 예상 총 상품 수: {expected_total}개")
            
            show_count = max(expected_total, 50)
            self.logger.info(f"🔧 reDrawTable({show_count}) 호출...")
            
            try:
                self.driver.execute_script(f"reDrawTable({show_count});")
                self.logger.info(f"✅ reDrawTable({show_count}) 호출 완료")
                time.sleep(5)
                ...
                if len(tables) >= 2:
                    ...
                    self.logger.info(f"🔍 발견된 상품 링크: {len(product_links)}개")
                    
                    if len(product_links) > 5:
                        self.logger.info(f"✅ 성공: {len(product_links)}개 상품이 한 페이지에 표시됨")
                        return True
                        
            except Exception as e:
                self.logger.info(f"⚠️ reDrawTable 함수 호출 실패: {e}")
            
            return False
            
        except Exception as e:
            self.logger.info(f"❌ 전체 상품 표시 실패: {e}")
            return False
    ...
    def extract_all_products_from_page(self):
        ...
        try:
            tables = self.driver.find_elements(By.TAG_NAME, "table")
            self.logger.info(f"📋 테이블 {len(tables)}개 발견")
            
            if len(tables) >= 2:
                product_table = tables[1]
                rows = product_table.find_elements(By.TAG_NAME, "tr")
                
                self.logger.info(f"📋 상품 테이블에서 {len(rows)}개 행 검사")
                    ...
                    if len(cells) >= 3:
                        ...
                        if product_links:
                            ...
                            if match:
                                ...
                                self.logger.info(f"🔍 상품 발견: {product_name}")
        
        except Exception as e:
            self.logger.info(f"❌ 상품 추출 중 오류: {e}")
        
        self.logger.info(f"📊 총 {len(products)}개 상품 추출 완료")
        return products
    ...
    def extract_detailed_product_info(self, product_info):
        """상세 페이지 정보 추출"""
        try:
            self.logger.info(f"🔍 {product_info['name']} 상세 정보 수집...")
            ...
            self.logger.info(f"✅ {product_info['name']} 완료")
            return detailed_product
            
        except Exception as e:
            self.logger.info(f"❌ {product_info['name']} 실패: {e}")
    ...
    def parse_detail_page_improved(self, product_info):
        ...
        self.logger.info(f"📝 {product_info['name']} 상세 정보 파싱 중...")
        ...
        try:
            ...
            self.logger.info(f"상세 정보 파싱 중 오류: {e}")
    ...
    def extract_structured_info(self):
        ...
        try:
            self.logger.info("🔍 상세 정보 추출 시작 (강화된 다중 전략 접근법)...")
                ...
                if '가입대상' in text and len(text) < 100:
                    ...
                    if match and not info['join_target']:
                        ...
                        if len(target) > 2 and target not in [':', '-', ')', '(']:
                            ...
                            self.logger.info(f"✅ 기본방법 - 가입대상: {target}")
                ...
                if '가입기간' in text and len(text) < 100:
                    ...
                    if match and not info['join_period']:
                        ...
                        if len(period) > 2 and period not in [':', '-', ')', '(']:
                            ...
                            self.logger.info(f"✅ 기본방법 - 계약기간: {period}")
                ...
                if '가입금액' in text and len(text) < 100:
                    ...
                    if match and not info['join_amount']:
                        ...
                        if len(amount) > 2 and amount not in [':', '-', ')', '(']:
                            ...
                            self.logger.info(f"✅ 기본방법 - 가입금액: {amount}")
                ...
                if '가입방법' in text and len(text) < 100:
                    ...
                    if match and not info['join_method']:
                        ...
                        if len(method) > 2:
                            ...
                            self.logger.info(f"✅ 기본방법 - 가입방법: {method}")
                ...
                if '과세우대' in text and len(text) < 100:
                    ...
                    if match and not info['tax_benefit']:
                        ...
                        if tax_text and '해당없음' not in tax_text and '없음' not in tax_text:
                            ...
                            self.logger.info(f"✅ 기본방법 - 세제혜택: {tax_text}")
                ...
                if '예금자보호' in text and len(text) < 100:
                    ...
                    if match and not info['deposit_protection']:
                        ...
                        if len(protection) > 2:
                            ...
                            self.logger.info(f"✅ 기본방법 - 예금자보호: {protection}")
            ...
            try:
                            ...
                            if len(clean_match) > 2 and clean_match not in [':', '-', ')', '(', '※']:
                                ...
                                if field == 'tax_benefit':
                                    if '해당없음' not in clean_match and '없음' not in clean_match:
                                        ...
                                        self.logger.info(f"✅ 정규식방법 - {field}: {clean_match}")
                                        break
                                else:
                                    info[field] = clean_match
                                    self.logger.info(f"✅ 정규식방법 - {field}: {clean_match}")
                ...
                self.logger.info(f"전체 페이지 텍스트 분석 중 오류: {e}")

            # 전략 4: 더 넓은 범위의 요소 검색
            self.logger.info("🔍 확장된 요소 검색...")
                                    ...
                                    if len(potential_value) > 2 and potential_value not in [':', '-', ')', '(']:
                                        if field == 'tax_benefit':
                                            if '해당없음' not in potential_value and '없음' not in potential_value:
                                                ...
                                                self.logger.info(f"✅ 확장검색 - {field}: {potential_value}")
                                                break
                                        else:
                                            info[field] = potential_value
                                            self.logger.info(f"✅ 확장검색 - {field}: {potential_value}")
                                            ...
                                            if len(value) > 2:
                                                if field == 'tax_benefit':
                                                    if '해당없음' not in value and '없음' not in value:
                                                        info[field] = value
                                                        self.logger.info(f"✅ 속성검색 - {field}: {value}")
                                                        break
                                                else:
                                                    info[field] = value
                                                    self.logger.info(f"✅ 속성검색 - {field}: {value}")
            ...
            self.logger.info(f"구조화된 정보 추출 중 오류: {e}")
        
        # 최종 결과 요약
        valid_info_count = sum(1 for value in info.values() if value)
        if valid_info_count > 0:
            self.logger.info(f"📊 구조화된 정보 추출 완료 ({valid_info_count}개 항목):")
            korean_names = {
                'join_target': '가입대상',
                'join_period': '계약기간', 
                'join_amount': '가입금액',
            ...
            for key, value in info.items():
                if value:
                    self.logger.info(f"  {korean_names.get(key, key)}: {value}")
        else:
            self.logger.info("📊 구조화된 정보 추출 완료 (0개 항목)")
            ...
            try:
                ...
                self.logger.info(f"  HTML 구조: 테이블 {len(tables)}개, div {len(divs)}개, span {len(spans)}개")
                
            except Exception as debug_error:
                self.logger.info(f"  디버깅 정보 수집 실패: {debug_error}")
    ...
    def extract_preferential_conditions(self):
        ...
        try:
            ...
            self.logger.info(f"우대조건 추출 중 오류: {e}")
    ...
    def extract_period_rates(self):
        ...
        try:
            ...
            self.logger.info(f"기간별금리 추출 중 오류: {e}")
    ...
    def crawl_category(self, category, url_code):
        ...
        try:
            ...
            if not show_all_success:
                self.logger.info("⚠️ 전체 상품 표시 실패, 현재 페이지 상품만 크롤링")
            
            all_products = self.extract_all_products_from_page()
            self.logger.info(f"📋 {category}에서 총 {len(all_products)}개 상품 발견")
            
            if len(all_products) == 0:
                self.logger.info(f"❌ {category}에서 상품을 찾을 수 없습니다")
                return []
            ...
            for i, product_info in enumerate(all_products):
                ...
                try:
                    ...
                    self.logger.info(f"✅ [{i + 1}/{len(all_products)}] {product_info['name']} 완료")
                    
                except Exception as product_error:
                    self.logger.info(f"❌ [{i + 1}/{len(all_products)}] {product_info['name']} 실패: {product_error}")
                    basic_product = self.create_basic_product(product_info)
                    category_products.append(basic_product)
                
                time.sleep(1.5)
            
            self.logger.info(f"🎉 {category} 크롤링 완료!")
            self.logger.info(f"📊 수집 결과: {len(category_products)}개 상품")
            
            return category_products
            
        except Exception as e:
            self.logger.info(f"❌ {category} 크롤링 중 치명적 오류: {e}")
    ...
    def crawl_period_rates(self, category, deop_dv):
        ...
        try:
            ...
            self.logger.info(f"🔄 기간별 금리 페이지로 이동: {rate_url}")
            ...
            if not self.wait_for_iframe_complete():
                self.logger.info("❌ 기간별 금리 페이지 로딩 실패")
                return {}
            
            self.logger.info("🔍 페이지 전체 구조 분석 중...")
            ...
            for selector in selectors:
                try:
                    ...
                    if elements:
                        ...
                        self.logger.info(f"✅ 상품 선택 요소 발견 ({selector})")
                        break
                except:
                    continue
            ...
            if not select_element:
                ...
                self.logger.info(f"📋 페이지 내 모든 select 요소: {len(all_selects)}개")
                
                for i, select in enumerate(all_selects):
                    options = select.find_elements(By.TAG_NAME, "option")
                    if len(options) > 3:
                        select_element = select
                        self.logger.info(f"🎯 상품 선택으로 추정되는 select 요소 선택")
                        break
            
            if not select_element:
                self.logger.info("❌ 상품 선택 드롭다운을 찾을 수 없습니다")
                return {}
            
            options = select_element.find_elements(By.TAG_NAME, "option")
            self.logger.info(f"📋 {len(options)}개 상품 옵션 발견")
            ...
            for i, option in enumerate(options):
                ...
                if not product_code or product_code == '' or product_name in ['선택하세요', '']:
                    self.logger.info(f"⏭️ 건너뛰기: \"{product_name}\" (값: \"{product_code}\")")
                    continue
                ...
                try:
                    # 상품 선택
                    self.logger.info(f"🔄 상품 선택: {product_code}")
                    ...
                    self.logger.info("⏳ 조회 후 테이블 업데이트 대기 중...")
                    time.sleep(6)
                    ...
                    if rate_table:
                        ...
                        self.logger.info(f"✅ {product_name}: {len(rate_table)}개 기간별 금리 추출 완료")
                    else:
                        self.logger.info(f"⚠️ {product_name}: 기간별 금리 데이터 없음")
                        
                except Exception as e:
                    self.logger.info(f"❌ {product_name} 기간별 금리 추출 실패: {e}")
                
                time.sleep(2)
            
            self.logger.info(f"🎉 {category} 기간별 금리 추출 완료: {len(period_rates_data)}개 상품")
            return period_rates_data
            
        except Exception as e:
            self.logger.info(f"❌ {category} 기간별 금리 추출 중 오류: {e}")
            return {}
    
    def extract_rate_table_from_page(self, product_name):
        """페이지에서 기간별 금리 테이블 추출 - 전체 테이블 정보 누락 없이 추출"""
        self.logger.info(f"🔍 {product_name} 금리 테이블 전체 추출 시작")
        ...
        for table_index, table in enumerate(tables):
            ...
            if len(rows) < 2:
                self.logger.info(f"  Table {table_index}: 행이 부족함 (건너뛰기)")
                continue
            ...
            if not has_rate_terms:
                self.logger.info(f"  Table {table_index}: 금리 관련 키워드 없음 (건너뛰기)")
                continue
            ...
            if not table_data or len(table_data) < 2:
                self.logger.info(f"  Table {table_index}: 유효한 데이터 없음 (건너뛰기)")
                continue
            ...
            self.logger.info(f"  Table {table_index}: 헤더 분석 - {header}")
            ...
            if any(exclude_conditions):
                self.logger.info(f"  Table {table_index}: 제외 조건에 해당함 (불필요한 테이블)")
                continue
            ...
            if not has_period_or_rate:
                self.logger.info(f"  Table {table_index}: 기간/금리 데이터 없음 (건너뛰기)")
                continue
            
            # 유효한 금리 테이블로 판단 - 전체 정보 보존
            self.logger.info(f"  Table {table_index}: ✅ 유효한 금리 테이블로 판정!")
            ...
            self.logger.info(f"  ✅ Table {table_index} 전체 추출 완료:")
            self.logger.info(f"    - 총 행수: {len(table_data)}개")
            self.logger.info(f"    - 데이터 행수: {len(non_empty_rows)}개") 
            self.logger.info(f"    - 최대 열수: {max_columns}개")
            
            # 전체 테이블 구조 출력 (모든 행)
            self.logger.info(f"  📋 전체 테이블 구조:")
            for i, row_data in enumerate(table_data):
                if i < 10:  # 처음 10행만 출력
                    formatted_cells = [f'"{cell}"' for cell in row_data]
                    self.logger.info(f"    Row {i}: {formatted_cells}")
                elif i == 10 and len(table_data) > 10:
                    self.logger.info(f"    ... (총 {len(table_data)}개 행)")
                    break
        
        if rate_table_data:
            total_rows = sum(table['총행수'] for table in rate_table_data)
            total_data_rows = sum(table['비어있지않은행수'] for table in rate_table_data)
            self.logger.info(f"🎉 {product_name} 유효한 금리 테이블 {len(rate_table_data)}개 추출 성공!")
            self.logger.info(f"📊 전체 {total_rows}개 행 (데이터 {total_data_rows}개 행) 완전 보존")
        else:
            self.logger.info(f"⚠️ {product_name}: 유효한 금리 테이블을 찾을 수 없음")
    ...
    def merge_period_rates(self, products, period_rates_data):
        ...
        for rate_name, rate_data in period_rates_data.items():
            ...
            if matched_product:
                ...
                self.logger.info(f"✅ {matched_product['상품명']} ← {rate_name} ({len(rate_data)}개 기간)")
            else:
                self.logger.info(f"⚠️ 매칭 실패: {rate_name}")
    ...
    def crawl_all_complete(self):
        ...
        self.logger.info("📋 1단계: 상품 기본정보 + 상세정보 수집")
        self.logger.info("📊 2단계: 기간별 금리 페이지에서 금리 테이블 수집")
        self.logger.info("🔗 3단계: 기간별 금리 정보 병합")
        self.logger.info("⏱️ 예상 소요 시간: 25-35분")
        ...
        try:
            ...
            self.logger.info("✅ iframe 발견 완료 - 이제 3단계 크롤링을 시작합니다")
            ...
            self.logger.info(f"📊 총 수집 상품: {len(self.products)}개")
            self.logger.info(f"💰 목돈만들기: {len(mokdon_making)}개")
            self.logger.info(f"💰 목돈굴리기: {len(mokdon_rolling)}개")
            self.logger.info(f"📈 목표 달성률: {round((len(self.products) / 57) * 100)}%")
            self.logger.info(f"🔢 기간별 금리 병합: {making_merge_count + rolling_merge_count}개 상품")
            self.logger.info(f"⚠️ 오류 발생: {len(self.errors)}건")
            
            if self.errors:
                self.logger.info("❌ 오류 목록:", self.errors)
            ...
            self.logger.info(f"❌ 전체 크롤링 중 치명적 오류: {e}")
            return None
        finally:
            if self.driver:
